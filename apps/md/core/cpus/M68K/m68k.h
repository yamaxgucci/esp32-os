...
// 16/32 bits acces to RAM/ROM
	...
	/*
	 * ArgonOS: the cartridge is a pointer to the image the loader already read,
	 * so there is no 8 MB static buffer and no copy.  Work RAM stays a static
	 * array - 64 KB is cheap and it is touched on nearly every instruction.
	 */
	extern unsigned char *ROM_DATA;
	extern unsigned char M68K_RAM[];
#else
