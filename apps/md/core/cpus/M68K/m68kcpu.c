...
/*
 * ArgonOS: the frame loop counts master clocks from zero every frame, so the
 * CPU's own counter has to be rebased by the same amount or it runs away.  Kept
 * here because m68k is static to this file and the platform has no business
 * including m68kcpu.h.
 */
void m68k_frame_end(int master_cycles)
{
