...
unsigned char *ROM_DATA = 0;          // points at the loaded cartridge image
...
/*
 * ArgonOS: take ownership of the image the caller loaded rather than copying it
 * into a static 8 MB array.  The 68000 fetch macros read the ROM byte-swapped
 * (FETCH8ROM xors the address), so the swap still has to happen - it just
 * happens in the caller's buffer, which from here on belongs to the emulator.
 */
