...
#ifdef ARGON_TARGET
/* Guest heap is 5 MiB (same class as MD); 6 MiB zone does not fit. */
...
#if !defined(_WIN32) && !defined(__MACOSX__) && !defined(__DJGPP__) && !defined(ARGON_TARGET)
...
#ifdef ARGON_TARGET
    /* Message already went to stderr via vfprintf. */
#else
