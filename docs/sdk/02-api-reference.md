...
Текущая версия ABI — **0.5**. Правила совместимости и то, как приложение
...
| [`dev`](#dev--устройства) | ✅ | реестр устройств, `/dev`, `ioctl` |
...
системного. Собранное против 0.1 работает на 0.2; собранное против 0.5 на 0.4 —
...
Полный пример — `apps/devs/devs.c`; он же проверяет всё перечисленное и
возвращает код, так что `run` + `errorlevel` — это тест.
...
Ни `probe` по таблице ID, ни 1-Wire, ни TWAI/CAN, ни RMT, ни MCPWM, ни I2S.
