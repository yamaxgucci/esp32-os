...
| [`power`](#power--частота-экран-и-оповещение) | ✅ 0.37 | что установлено, о чём система собирается попросить, и ответ, от которого зависит, останется ли процесс жив |
...
Появилась в **0.35**, правило снятия — в **0.36**, ступень `cruise` — в **0.37**.
Три вызова, и ни один из них ничего не решает за приложение.
...
static inline ag_err_t ag_power_declare(ag_power_fitness_t fitness, const char *why);
```

Система не меняет частоту и не гасит экран молча: сначала она объявляет, что
собирается сделать, и ждёт. **Кто именно попросил — важно**, и это лежит в
`status()`:

| `p.cause` | Что это | Чем это грозит |
|---|---|---|
| `AG_POWER_AUTO` | таймер простоя | ничем. Совещательно: не ответишь — ничего не случится |
| `AG_POWER_USER` | человек набрал `power eco` | приказ. **Приложение, не подтвердившее, что рассчитано на новый режим, снимается** |

Подтвердить — значит ответить `AG_POWER_OK` (работаю дальше) или
`AG_POWER_PARKED` (остановился сам) за отведённое окно (`p.grace_ms`, обычно
500 мс):
...
`AG_POWER_UNFIT` — это признание, а не вето: режим включится, а этот процесс
будет снят, и его `why` увидит человек за консолью. Ровно то же будет с тем, кто
промолчал, — с той разницей, что сказать про него будет нечего.

**Ступень `AG_POWER_CRUISE` (160 МГц) — исключение из всего вышесказанного.**
Её система берёт сама, пока никто не сказал, что ему нужна полная частота:
молчание там считается согласием, потому что шина периферии на этой ступени не
двигается и ломаться нечему, кроме арифметики на пределе. Приложение, которому
её не хватает, обязано сказать об этом само:

```c
int ag_main(int argc, char **argv)
{
    ag_power_declare(AG_POWER_FIT_FULL_ONLY, "22 kHz tract");
    ...
```

Вызов действует **немедленно**: частота поднята к моменту возврата, а не к
следующему тику. Если этого не сделать, программа будет считать на двух третях
скорости и никто об этом не узнает — единственное место в этой подсистеме, где
такое возможно, и цена входного билета в него — одна строка.

Кто не хочет опрашивать каждый кадр, объявляет один раз:

| `ag_power_declare(…)` | Значение |
|---|---|
| `AG_POWER_FIT_ANY` | любой режим годится, больше не спрашивать. Никогда не снимается из-за смены режима |
| `AG_POWER_FIT_FULL_ONLY` | нужна полная частота. Таймер простоя её тогда не трогает; команда человека снимает этот процесс и печатает причину |
| `AG_POWER_FIT_ASK` | по умолчанию: отвечаю сам, каждый раз |

Объявление снимается вместе с процессом, как любой другой ресурс.

`cpu_mhz` в структуре — живое чтение с машины; `pending_cpu_max_mhz` — то, чем
частота станет, потому что решать по названию режима нельзя: синтезатор, который
измерил свою загрузку, знает про 80 МГц, а про «eco» не знает ничего.

Что снятия **не** вызывает: подъём обратно (`power full`), выключение экрана само
по себе, `power cruise` для того, кто просто молчал, всё, что делает таймер
простоя, и процесс, который ещё грузится.

Пример на все пять поведений — [`apps/pwr/pwr.c`](../../apps/pwr/pwr.c), разбор и
цена — [`11-power.md`](../11-power.md).
