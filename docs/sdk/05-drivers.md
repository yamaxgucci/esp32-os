...
* Драйвер панели с управляемой подсветкой отвечает на
  `AG_IOC_DISPLAY_BACKLIGHT` (аргумент — `uint8_t`, 0..100). Это то, чем
  `power screen off` гасит настоящий экран: пин подсветки принадлежит модулю, а
  не ядру. У кого света нет — отвечает `-AG_ENOTSUP`, и это не ошибка, а ответ:
  `power` тогда честно пишет, что гасить оказалось нечем. Пример —
  `apps/ili9341/ili9341.c`, там же контроллер панели усыпляется целиком.
