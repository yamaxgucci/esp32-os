...
## Экран на слот — сделано, и что мешает следующему шагу

**Сделано (10 сентября 2026, `f8daade`):** у каждого слота свой экран консоли
(`ag_console_use_slot`), переключение больше не стирает то, что сказала
программа. Экраны держит консоль: по ним ходит `resize`, и все тридцать с
лишним мест в ядре берут экран через `ag_console_screen()`. Создаются лениво;
если памяти нет — слоты снова делят один, то есть медленная версия прежнего
поведения. Проверка: `tools/slotscreens.ps1`.

**Не сделано: вывод по слоту пишущего.** Сегодня вывод приложения, у которого
нет консоли, уходит в журнал (`api.c`, `bg_stdout_write`) — обход, поставленный
ровно потому, что экран был один. С экранами на слот его хотелось бы снять,
и попытка показала, что почва не готова:

* **фоновый процесс вообще не привязан к слоту.** `run /b` запускает его с
  `AG_SPAWN_NO_SESSION`, а последующая привязка в `spawn_in_slot` в замере не
  срабатывала: `ag_session_slot_of_any()` отвечала «слота нет» на протяжении
  всего прогона. Пока процесс не знает своего слота, спрашивать «чей это
  экран» не у кого. Правильное место для ответа — запись процесса
  (`proc_t`), заполняемая при привязке, а не поиск по таблице слотов,
  который вдобавок идёт без синхронизации с тем, кто её меняет.
* **журнал уходит в последовательный порт мимо экрана.** Поэтому `vtdump`,
  который восстанавливает экран проигрыванием этого порта, показывает
  журнальные строки как содержимое экрана. Любой тест про «что на экране
  слота» ломается, если в это время что-то пишет в журнал.

Порядок, если браться снова: сначала слот в `proc_t`, потом маршрутизация
вывода, потом ввод (очередь на слот) — и только после этого шелл в другом
слоте, то есть «MS-DOS Prompt» в окне.
