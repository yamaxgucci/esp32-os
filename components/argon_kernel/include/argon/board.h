...
/*
 * Pinout and default rate for a future I2S .SYS driver.  The kernel only
 * registers /dev/pcmnull; virt/I2S devices come from loadable modules.
 */
typedef struct {
    char     driver[12]; /* reserved / hint for future PCMI2S.SYS ("i2s", …) */
    int16_t  bclk;
    int16_t  ws;
    int16_t  dout;
