/*
 * ArgonOS - a boot HID keyboard, as far as everything else is concerned.
 *
 * A port hands over HID reports; this turns them into the console events the
 * terminal decoder already produces, and from there nothing can tell the
 * difference between a key pressed on a keyboard attached to the board - over
 * Bluetooth or USB - and one typed into a serial terminal on the other side of
 * the world.  That is the whole design: the shell, the editor and the file
 * manager gain a keyboard without one line of change.
 *
